...
async function scrapeItem(url, cb) {
  ...
  let itemPromise = (url) => new Promise(async(resolve, reject) => {
        let product = await page.evaluate(async () => {
            ...
            let items = document.querySelectorAll('.pdp-header__product-name, .page-title > span, [class*="product-title"], [class*="productName"], [class*="product-name"], [class*="productTitle"], .product-detail__title h1, [class*="mainColumn-"] div [class*="title-"], #itemTitle');
            items.forEach(item => {
                if (item.innerText) {
