...
async function scrapeItem(url, cb) {
  ...
  const urlShort = url.substring(0, url.indexOf('?') === -1 ? url.length : url.indexOf('?'));
  try {
    ...
    console.log(`Navigating to ${urlShort}...`);
    await page.goto(url, { waitUntil: 'networkidle2' }); // wait till html is loaded

    // function - promise to scrape and return item object
    const itemPromise = (urlToget) => new Promise(async (resolve, reject) => {
      const product = await page.evaluate(async () => {
        const results = {};
        // Scrape title|name of product
        const items = document.querySelectorAll(`.pdp-header__product-name, .page-title > span, [class*="product-title"], 
        ...
        const img = document.querySelectorAll(`.imgTagWrapper img, [class*="productImage"], .static-product-image, 
        ...
        const ebayImg = document.querySelector('#viEnlargeImgLayer_layer_fs_thImg0 > table > tr> td > div > img');
        if (ebayImg) {
          results.imgURL = ebayImg.src;
        }

        // Scrape price
        const prices = document.querySelectorAll(`[class*="price"], [class*="Price"], [class*="price__total"], 
                                                    .price__total-value price__total--on-sale`);
        const amazonPrice = document.querySelector('#priceblock_ourprice');
        const amazonPrice1 = document.querySelector('#priceblock_saleprice');
        const ebayPrice = document.querySelector('#prcIsum');
        const bestBuyPrice = document.querySelector('.screenReaderOnly_3anTj');
        ...
        // Scrape description
        const description = document.querySelectorAll(`.product-description-blurb__text, [class*="description"], [class*="Description"], 
        ...
        description.forEach((item) => {
          ...
          } else if (item.innerText && !results.description) {
      ...
      product.url = urlToget;
      resolve(product);
      reject('Unable!');
    });

    const item = await itemPromise(urlShort);
    cb(item);
    await cleanup();
  } catch (err) {
